...
use orchestrator_core::model::{ExecBlockResult, ExecutorSpec, JobStatus, StageConfig, StageKind};
        ...
        let mut result = match (&lease.kind, &lease.config) {
            (StageKind::ExecBlock, StageConfig::ExecBlock(spec)) => {
                ...
                if outcome.decision == Decision::Block {
                    ...
                    exec_runner::run_exec_block(&lease, &args.runs_root, &args.agent_id).await
                }
            }
            _ => ExecBlockResult {
                run_id: lease.run_id.clone(),
