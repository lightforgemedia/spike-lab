...
impl OrchestratorService {
    ...
    pub async fn claim_job(&self, agent_id: String) -> anyhow::Result<Option<JobLease>> {
        ...
        let job_id = job_id_str(&job)?;
        let lease_token = Uuid::new_v4().to_string();
        let now = now_ms();
        let lease_expires_at_ms = now + self.lease_ms;

        // Update job -> running with lease.
        let q_update = r#"
            UPDATE $job
            SET status = 'running',
    ...
    async fn update_run_status(&self, run_id: &str) -> anyhow::Result<()> {
        ...
        if fail_count > 0 {
            let q = r#"UPDATE $run SET status = 'failed' RETURN NONE;"#;
            self.db
                .inner()
                .query(q)
        ...
        if remaining == 0 {
            let q = r#"UPDATE $run SET status = 'succeeded' RETURN NONE;"#;
            self.db
                .inner()
                .query(q)
                .bind(("run", thing_from_id(run_id)))
